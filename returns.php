...
if(isset($_POST['returnbook']))
{
    ...
    $borrow_query = "DELETE from bookLoan where `transactionNum` <= '$borrow_returnbook' and `transactionNum` >= '$borrow_returnbook'";
    deletefromtable($borrow_query);
    $test = $user_data['cardID'];
    $query = "SELECT * FROM bookLoan where `cardID` <= $test and `cardID` >= $test ";
    $search_result = filterTable($query);
...
<!-- new additions -->
        <br>
        <h2>Return a Book</h2>
        <p>Insert transaction number to return a book</p>
