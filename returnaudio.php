...
if(isset($_POST['returnbook']))
{
    ...
    $borrow_query = "DELETE from audioLoan where `transactionNum` <= '$borrow_returnbook' and `transactionNum` >= '$borrow_returnbook'";
    deletefromtable($borrow_query);
    $test = $user_data['cardID'];
    $query = "SELECT * FROM audioLoan where `cardID` <= $test and `cardID` >= $test ";
    $search_result = filterTable($query);
    ...
    <body>
     ...
     <br>
        <a href="index.php">Back to home</a><br><br>
         <h1>Audio currently checked out</h1>
...
<!-- new additions -->
        <br>
        <h2>Return a Book</h2>
        <p>Insert transaction number to return a album</p>

        <form action="returnaudio.php" method="post">
